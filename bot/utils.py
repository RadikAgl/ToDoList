...
from bot.data_exchanger import request_new_access_token
from bot.keyboards import get_keyboard_tasks_menu
...
def make_task_msgs(tasks):
    return [(f"<b>{task['title']}</b>\n"
             ...
             get_keyboard_tasks_menu(task['id'])) for task in list(tasks)]
